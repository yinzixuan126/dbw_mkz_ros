...
void JoystickDemo::recvJoy(const sensor_msgs::Joy::ConstPtr& msg)
{
  ...
  // Gear
  if (msg->buttons[3]){
    joy_data_.gear_cmd = GEAR_PARK;
  }else if(msg->buttons[1]){
    joy_data_.gear_cmd = GEAR_REVERSE;
  }else if(msg->buttons[0]){
    joy_data_.gear_cmd = GEAR_DRIVE;
  }else if(msg->buttons[2]){
    joy_data_.gear_cmd = GEAR_NEUTRAL;
  }else{
    joy_data_.gear_cmd = GEAR_NONE;
  }
  ...
  if (msg->axes[6] != last_joy_.axes[6]) {
    switch (joy_data_.turn_signal_cmd) {
      case TURN_NONE:
        if (msg->axes[6] < -0.5) {
          joy_data_.turn_signal_cmd = TURN_RIGHT;
        } else if (msg->axes[6] > 0.5) {
          joy_data_.turn_signal_cmd = TURN_LEFT;
        }
        break;
      case TURN_LEFT:
        if (msg->axes[6] < -0.5) {
